...
export default function GachaHubPage() {
        ...
        const load = async () => {
            try {
                ...
                if (json.success) {
                    ...
                    // Build unique categories from machines
    ...
    // Legacy gacha machine card shown at the top
    const GACHA_X = { id: "__gacha_x__", name: "สุ่มตัว X", href: "/gacha" };
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-zinc-950">
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                {/* Main Content — full width, category tabs inside header */}
                <div className="flex-1">
                    <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-border overflow-hidden">
                        ...
                        <div className="border-b border-border py-3 px-5 flex flex-wrap items-center gap-2">
                            ...
                            {/* Built-in machine quick links */}
                            <span className="w-px h-5 bg-border mx-1" />
                            <Link href="/gacha"
                                className="px-3 py-1 rounded-full text-sm font-medium border border-[#1a56db]/60 text-[#1a56db] hover:bg-[#1a56db] hover:text-white transition-colors">
                                สุ่มตัว X
                            </Link>
                            <Link href="/gacha-grid"
                                className="px-3 py-1 rounded-full text-sm font-medium border border-emerald-500/60 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-colors">
                                สุ่มกงล้อ 3×3
                            </Link>
                        </div>
